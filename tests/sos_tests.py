...
class BaseSoSReportTest(BaseSoSTest):
    ...
    def _execute_sos_cmd(self):
        ...
        """
        _cmd = './bin/sos report --batch --tmp-dir %s %s'
        exec_cmd = _cmd % (self.tmpdir, self.sos_cmd)
        self.cmd_output = process.run(exec_cmd, timeout=120)
    ...
    def assertFileCollected(self, fname):
        """Ensure that a given fname is in the extracted archive
        ...
        with open(fname, 'r') as lfile:
            for line in lfile.read().splitlines():
                if re.match(".*%s.*" % content, line, re.I):
                    matched = True
                    break
        assert matched, "Content '%s' does not appear in %s" % (content, fname)
...
class StageOneReportTest(BaseSoSReportTest):
    ...
    def test_no_new_kmods_loaded(self):
        ...
        :avocado: tags=stageone
        """
        self.assertEqual(self.sysinfo['pre']['modules'],
    ...
    def test_no_exceptions_during_execution(self):
        ...
        self.log.debug(os.listdir(self.get_name_in_archive("sos_logs/")))
